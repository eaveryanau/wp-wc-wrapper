...
//				'post_type'      => [ 'product', 'product_variation' ],
                    'post_type' => ['product'],
                    'posts_per_page' => -1,
                    'product_cat' => $data['category_slug']
                ]);
            } else {
                $products = array();
                $loop = new WP_Query([
...
static function createProduct($data)
{
    try {
        ...
        if (isset($data['image_new_name'])) {
            ...
            // Insert the attachment.
            $attach_id = wp_insert_attachment($attachment, $new_image, $post_id);
...
static function getCategories()
{
    try {
        ...
        $response = ['data' => '', 'error' => 'Internal error into ProductManager::getCategories()'];
    }
    return $response;
}
}